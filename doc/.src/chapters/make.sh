...
opt="--allow_refs_to_external_docs"
sphinx=0
publish=0
...
system doconce format pdflatex ${mainname} $preprocessor_opt $solutions $comments --latex_table_format=center --device=paper "--latex_code_style=default:vrb-blue1@sys:vrb[frame=lines,label=\\fbox{{\tiny Terminal}},framesep=2.5mm,framerule=0.7pt,fontsize=\fontsize{9pt}{9pt}]" $opt
...
system doconce format pdflatex ${mainname} $preprocessor_opt $no_solutions $comments --latex_table_format=center --device=screen "--latex_code_style=default:vrb-blue1@sys:vrb[frame=lines,label=\\fbox{{\tiny Terminal}},framesep=2.5mm,framerule=0.7pt,fontsize=\fontsize{9pt}{9pt}]" $opt
doconce latex_exercise_toc ${mainname}
doconce subst 'frametitlebackgroundcolor=.*?,' 'frametitlebackgroundcolor=blue!5,' ${mainname}.tex
system pdflatex ${mainname}
makeindex ${mainname}
...
system doconce format html ${mainname} $preprocessor_opt $no_solutions --html_style=solarized3 --html_output=${nickname}-$style --pygments_html_style=perldoc $comments $opt
...
system doconce format html ${mainname} $preprocessor_opt $no_solutions $comments --html_style=bootswatch_journal --html_code_style=inherit --html_output=${nickname} $opt
...
system doconce format html ${mainname} $preprocessor_opt $comments --html_style=bootswatch_journal --html_code_style=inherit --html_output=${nickname}-sol $opt
doconce replace 'P$d$' 'Pd' ${nickname}-sol.html
system doconce split_html ${nickname}-sol.html

if [ $sphinx -eq 1 ]; then
system doconce format sphinx ${mainname} $preprocessor_opt $no_solutions $comments --sphinx_keep_splits $opt
